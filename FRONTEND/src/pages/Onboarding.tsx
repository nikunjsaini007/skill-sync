...
import { ArrowRight, ArrowLeft, Check, Sparkles, AlertCircle, RefreshCw, Upload, X } from "lucide-react";
...
export default function Onboarding({
  ...
  const [college, setCollege] = useState(
    initialProfile
      ? (PRESET_COLLEGES.includes(initialProfile.college) ? initialProfile.college : "Other")
      : ""
  );
  const [customCollege, setCustomCollege] = useState(
    initialProfile && !PRESET_COLLEGES.includes(initialProfile.college) ? initialProfile.college : ""
  );
  const [avatar, setAvatar] = useState(initialProfile?.avatar ?? AVATAR_OPTIONS[0]);
  const [skillsOffered, setSkillsOffered] = useState<string[]>(initialProfile?.skillsOffered ?? []);
  const [skillsWanted, setSkillsWanted] = useState<string[]>(initialProfile?.skillsWanted ?? []);
  const [customOffered, setCustomOffered] = useState("");
  const [customWanted, setCustomWanted] = useState("");
  const [customOfferedSkills, setCustomOfferedSkills] = useState<string[]>(
    initialProfile?.skillsOffered.filter(
      s => !POPULAR_SKILLS.some(p => p.name.toLowerCase() === s.toLowerCase())
    ) ?? []
  );
  const [customWantedSkills, setCustomWantedSkills] = useState<string[]>(
    initialProfile?.skillsWanted.filter(
      s => !POPULAR_SKILLS.some(p => p.name.toLowerCase() === s.toLowerCase())
    ) ?? []
  );
  const [experience, setExperience] = useState<"Beginner" | "Intermediate" | "Advanced" | "Expert">(initialProfile?.experience ?? "Intermediate");
  const [interests, setInterests] = useState(initialProfile?.interests ?? "");
  const [learningGoals, setLearningGoals] = useState(initialProfile?.learningGoals ?? "");
  ...
  const addCustomSkill = (kind: "offered" | "wanted") => {
    const value = (kind === "offered" ? customOffered : customWanted).trim();
    if (!value) return setError("Please type a skill name first.");
    const list = kind === "offered" ? skillsOffered : skillsWanted;
    const customList = kind === "offered" ? customOfferedSkills : customWantedSkills;
    if (list.some(s => s.toLowerCase() === value.toLowerCase())) {
      return setError("That skill is already in your list.");
    }
    setError("");
    if (kind === "offered") {
      setSkillsOffered([...skillsOffered, value]);
      setCustomOfferedSkills([...customOfferedSkills, value]);
      setCustomOffered("");
    } else {
      setSkillsWanted([...skillsWanted, value]);
      setCustomWantedSkills([...customWantedSkills, value]);
      setCustomWanted("");
    }
  };

  const removeCustomSkill = (kind: "offered" | "wanted", skill: string) => {
    if (kind === "offered") {
      setSkillsOffered(skillsOffered.filter(s => s !== skill));
      setCustomOfferedSkills(customOfferedSkills.filter(s => s !== skill));
    } else {
      setSkillsWanted(skillsWanted.filter(s => s !== skill));
      setCustomWantedSkills(customWantedSkills.filter(s => s !== skill));
    }
  };
                  ...
                  <div className="flex gap-2 pt-1">
                    <input
                      ...
                      onKeyDown={e => {
                        if (e.key === "Enter") {
                          e.preventDefault();
                          addCustomSkill("offered");
                        }
                      }}
                      placeholder="Type a custom skill you can teach..."
                      className="flex-1 px-3 py-2 rounded-lg bg-brand-bg border border-brand-border focus:border-emerald-500/50 focus:outline-none text-xs text-slate-200 placeholder:text-slate-600 transition-colors"
                    />
                    <button
                      type="button"
                      onClick={() => addCustomSkill("offered")}
                      className="px-3.5 py-2 rounded-lg text-xs font-bold bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 hover:bg-emerald-500/25 transition-colors"
                    >
                      Add Skill
                    </button>
                  </div>

                  {customOfferedSkills.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {customOfferedSkills.map(skill => (
                        <span key={skill} className="flex items-center gap-1 rounded-lg bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 px-2.5 py-1 text-xs font-semibold">
                          {skill}
                          <button
                            type="button"
                            onClick={() => removeCustomSkill("offered", skill)}
                            aria-label={`Remove ${skill}`}
                            className="text-emerald-300/60 hover:text-red-400 transition-colors"
                          >
                            <X className="w-3 h-3" />
                          </button>
                        </span>
            ...
            <div className="flex items-center gap-2">
              {isEdit && onCancel && (
                <button
                  type="button"
                  onClick={onCancel}
                  className="px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-400 hover:text-red-400 flex items-center gap-1"
                >
